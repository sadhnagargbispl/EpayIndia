...
public partial class KitMaster : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                AddKit.Attributes.Add("onclick", DisableTheButton(Page, AddKit));
                txtSearch.Text = "";
    ...
    public void BindData()
    {
        ...
        try
        {
            ...
            {
                Condition = Condition + " And " + ddlSearchFields.SelectedValue + "  Like   '%" + ClearInject(txtSearch.Text) + "%' ";
            }
            if (String.Equals(ddlSearchFields.SelectedItem.Text.ToLower(), "status"))
            {
                if (!String.IsNullOrEmpty(txtSearch.Text))
                {
                    ...
                    sql = objDal.IsoStart + " select * from  v#kITMASTER Where 1=1  AND Status = '" + status.ToString() + "'" + objDal.IsoEnd;
                }
            }
            else
            {
                sql = objDal.IsoStart + " select * from  v#kITMASTER Where 1=1  " + Condition + objDal.IsoEnd;
            }
    ...
    protected void DeleteGroup(object sender, EventArgs e)
    {
        try
        {
            ...
            KitId = ((Label)GVRw.FindControl("LblNewsID")).Text;
            string Sql = "Update M_KitMaster SET ActiveStatus='N',LastModified='De-Activated by " + Session["UserName"] + " at " + DateTime.Now.ToString() + "' WHERE KitId='" + KitId.ToString() + "' AND RowStatus='Y'";
    ...
    protected void BtnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("AddKit.aspx", false);
        }
        catch (Exception Ex)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alertMessage", "alert('" + Ex.Message + "')", true);
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=KitMaster-" + DateTime.Now.ToShortDateString() + ".xlsx");
